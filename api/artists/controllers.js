...
const controllers = {
  ...
  getOne: (req, res) => {
    ...
    const sql = `SELECT * FROM artists WHERE ArtistId =  ${req.params.item}`;
  ...
  getOneByName: (req, res) => {
    ...
    const sql = `SELECT * FROM artists WHERE Name =  "${req.params.item}"`;
    db.all(sql, (err, rows) => {
      if (err) {
        res.status(400).json({
          "error": err.message
  ...
  update: (req, res) => {
    ...
    db.all(sql, (err, rows) => {
      ...
      if (rows.length === 0) return res.json({
        ...
        sql = `UPDATE artists SET Name="${re.name}" WHERE ArtistId = ${req.params.id}`;
