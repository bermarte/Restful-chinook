...
class Albums extends Component {
    ...
    async editItem(id, alb, artid) {
        ...
        const val_album = (this.album === '')
            ? alb
            : this.album;
        const val_art_id = (this.artist === 0)
            ? artid
            : this.artist;
        ...
        const Home = () => (
            <Row>
                <Table striped bordered hover>
                    ...
                    <tbody>
                        {getAlbums.map((album, index) => <tr key={index + 1}>
                            ...
                            <td className="align-middle">
                                <ButtonGroup>
                                    <Button
                                        ...
                                        role="button"
                                        onClick={() => this.editItem(album.AlbumId, album.Title, album.ArtistId)}>
                                        <PencilIcon/>
                                    </Button>
                                    {/* to="/playlist/add" */}
                                    <Button
                                        className="btn btn-secondary btn-sm"
                                        role="button"
                                        onClick={() => this.deleteItem(album.AlbumId, index)}>X</Button>
                                    {/* to="/playlist/search" */}
                                </ButtonGroup>
                            </td>
            ...
            const AlbumListForm = () => {
                ...
                const saveAlbumlist = async(albumtitle, artistid) => {
                    ...
                    try {
                        ...
                        newState
                            .getAlbums
                            .push({AlbumId: newId, ArtistId: artistid, Title: albumtitle});
